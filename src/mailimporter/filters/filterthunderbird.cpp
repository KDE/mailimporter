...
{
    return QDir::homePath() + QLatin1StringView("/.thunderbird/");
}
...
void FilterThunderbird::import()
{
    ...
    if (!maildir.isEmpty()) {
        const QString mailDirThunderbird = maildir + QLatin1StringView("/Mail/Local Folders/");
...
{
    if ((file.endsWith(QLatin1StringView(".msf")) || file.endsWith(QLatin1StringView(".dat")) || file.endsWith(QLatin1StringView(".json"))
         || file.endsWith(QLatin1StringView(".html")))) {
        return true;
    }
    return false;
}
